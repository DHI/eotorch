...
class PointwiseBlock(nn.Module):
    def __init__(
        ...
        assert len(n_filters)==3, 'n_filters requires a list of exactly three values.'
        super().__init__()

        self.in_channels = in_channels
        self.n_filters = n_filters
        
        self.conv1 = Conv2d(in_channels, n_filters[0], kernel_size=(1,1), relu=True)
        self.conv2 = Conv2d(in_channels, n_filters[1], kernel_size=(1,1), relu=True)
        self.conv3 = Conv2d(in_channels, n_filters[2], kernel_size=(1,1), relu=False)
        self.conv_shortcut = Conv2d(in_channels, n_filters[2], kernel_size=(1,1), relu=False)
        self.relu = nn.ReLU(inplace=True)

    def forward(self, inputs : torch.Tensor):
        if self.in_channels == self.n_filters[-1]:
            shortcut = inputs
        else:
...
class SepConvBlock(nn.Module):
    def __init__(
        ...
        self.relu = nn.ReLU(inplace=True)
        self.sepconv1 = SeparableConv2d(in_channels, n_filters)
        self.sepconv2 = SeparableConv2d(n_filters, n_filters, relu=False)
        self.conv_shortcut = Conv2d(in_channels, n_filters, kernel_size=(1,1), relu=False)
...
class AuxModel(nn.Module):
    def __init__(
        ...
        self.residual = Conv2d(in_channels, n_filters, kernel_size=(1,1), bn=False, bias=True)
        self.reslayer1 = ResLayer(n_filters, n_filters)
        self.reslayer2 = ResLayer(n_filters, n_filters)
        self.reslayer3 = ResLayer(n_filters, n_filters)
        self.reslayer4 = ResLayer(n_filters, n_filters)
        self.output = nn.Conv2d(n_filters, 1, kernel_size=(1, 1), padding="same")
...
class ResLayer(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.conv1 = Conv2d(in_channels, out_channels, kernel_size=(1,1), bn=False, bias=True)
        self.dropout = nn.Dropout(0.5)
        self.conv2 = Conv2d(out_channels, out_channels, kernel_size=(1,1), bn=False, bias=True)
...
class XceptionSDB(nn.Module):
    ...
    def __init__(
        ...
        assert n_sepconv_filters > filters*2, 'The separable convolution filter size in \
            is smaller than the final filter size in the point-wise block.'
        filters = [n_filters, n_filters*2, n_sepconv_filters]
        ...
        self.entry_block = PointwiseBlock(in_channels, filters=[n_filters, n_filters*2, n_sepconv_filters])
        self.sepconvblock = self._make_sepconv_blocks()
        self.mean = Conv2d(n_sepconv_filters, 1, kernel_size=(1,1), relu=False, bn=False)
        self.variance = Conv2d(n_sepconv_filters, 1, kernel_size=(1,1), relu=False, bn=False)
        ...
        if not trainable:
            ...
            for param_mean, param_var in zip(self.mean.parameters(), self.variance.parameters()):
