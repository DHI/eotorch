...
from eotorch.layers.conv import Conv2d, ResBlock
...
class DeepResUNet(nn.Module):
    def __init__(
        ...
        super().__init__()
        filters = [n_filters]*4 if static_filters else list(reversed([n_filters//(2**i) for i in range(4)]))

        # Encoder
        self.input_conv = Conv2d(n_channels, filters[0], kernel_size=(5,5), padding='same', strides=(1,1))
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.conv = Conv2d(in_channels, out_channels, kernel_size=(1,1), padding='valid')
        self.resblock1 = ResBlock(in_channels, out_channels, kernel_sizes=[3, 3, 1])
        self.resblock2 = ResBlock(in_channels, out_channels, kernel_sizes=[3, 3, 1])
