...
from .utils import slice_image, get_outpath
...
def normalize(
    ...
    out_path = get_outpath(img_path, out_dir, out_path, subfolder='input', suffix='_norm')
    ...
    if out_res is not None:
        image, profile = resample(img_path, res=out_res, resampling='bilinear', indexes=band_list)
...
def sample_limits(
    ...
    with rst.open(img_path) as src:
        windows = slice_image(*src.shape, as_windows=True)
        sample_size = int(len(windows)*sample_size) if sample_size <= 1 else sample_size
        if sample_size > len(windows):
            raise ValueError('The number of samples specified exceed the total number of blocks.')
        sample_arr = np.zeros((sample_size, len(band_list), 256, 256), dtype=src.meta['dtype'])
